...
io.on('connection', (socket) => {
  ...
  socket.on('room:switch', async (room) => {
    ...
    if (room === 'vip' && !['owner','admin','moderator','vip'].includes(currentUser.role)) {
  ...
  socket.on('message:send', async (messageData) => {
    ...
    if (room === 'vip' && !['owner','admin','moderator','vip'].includes(user.role)) {
      socket.emit('room:error', { message: '⚠️ تحتاج عضوية VIP' }); return;
    }
    ...
    const message = {
      ...
      avatar:    user.avatar || '',
      replyTo:   (messageData.replyTo && typeof messageData.replyTo === 'object') ? { id: String(messageData.replyTo.id||'').slice(0,50), name: sanitizeName(messageData.replyTo.name||''), text: sanitizeText(messageData.replyTo.text||'').slice(0,100) } : null,
      reactions: {}
    };
  ...
  socket.on('message:react', (data) => {
    if (!currentUser) return;
    if (!checkRateLimit(socket.id, 'react', 20, 10000)) return;
    const { messageId, emoji, room } = data;
    const validEmojis = ['❤️','😂','😮','😢','😡','👍','👎','🔥','🎉','💯'];
    if (!validEmojis.includes(emoji)) return;
    const validR = ['general','friends','vip','games'];
    if (!validR.includes(room)) return;
    ...
    io.to(room).emit('message:reaction', { messageId, emoji, userId: socket.id, room });
  });

  // ─── مؤشر الكتابة ───────────────────────────────────────────────
  socket.on('user:typing', (data) => {
    if (!currentUser) return;
    socket.to(data.room || 'general').emit('user:typing', { userId: socket.id, name: currentUser.name });
  });
  socket.on('user:stopped-typing', (data) => {
    if (!currentUser) return;
    socket.to(data.room || 'general').emit('user:stopped-typing', { userId: socket.id });
  });
